...
; RUN: %llvm-as -o %t %s
; RUN: %opt -load-pass-plugin %pass -passes='function(souper),dce' -souper-enumerative-synthesis-max-instructions=1 -S -o - %s | %FileCheck %s
...
define i1 @syn_sg(i32 %x) #0 {
  %a = icmp sge i32 0, %x
  %b = xor i1 %a, true
  ; CHECK: icmp {{sle i32 1, %x|slt i32 0, %x}}
  ret i1 %b
}
